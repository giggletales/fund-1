/*
  ...
  1. New Tables
    - `account_metrics`
      - Stores real-time trading metrics for each MT5 account
      - Tracks balance, equity, drawdown, profit targets, consistency scores
    - `rule_violations`
      - Records all rule violations detected by the monitoring system
      - Links to specific accounts and users with severity levels
    - `notifications`
      - User notification system for alerts and updates
      - Supports different types and priority levels
    - `certificates`
      - Stores certificate generation records for passed challenges
      - Links certificates to users and accounts
    - `affiliates`
      - Affiliate program management
      - Tracks affiliate codes, commission rates, and earnings
    - `referrals`
      - Tracks referral relationships between affiliates and users
      - Records conversion status
    - `commissions`
      - Commission tracking for affiliate earnings
      - Links to payments and referrals
    - `payouts`
      - Payout requests and processing for affiliates
